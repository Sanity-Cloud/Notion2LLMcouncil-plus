...
document.getElementById('save').addEventListener('click', async () => {
  ...
  // Basic validation: Check for empty accelerators if we want to enforce all are set,
  // though allowing empty (unbound) is also valid. The user mentioned "empty-accelerator validation".
  // Let's just warn if any are empty but allow saving.
  const empties = fields.filter(f => !form[f]);
  if (empties.length > 0) {
    console.log('Some hotkeys are empty and will be unbound:', empties);
  }
